...
namespace constants {
template <class T>
static constexpr T pi = static_cast<T>(
    3.14159265358979323846264338327950288419716939937510582097494459);
}

void assert_compile_time() {
  {
...
void assert_runtime() {
  {
    const vec4 expected{-1, 0, 0, 1};
    const auto actual =
        make_rotation(vec3{0, constants::pi<float>, 0}) * vec4{1, 0, 0, 1};
    constexpr float epsilon = std::numeric_limits<float>::epsilon();
    assert(std::fabs(expected.x - actual.x) < epsilon);
    assert(std::fabs(expected.y - actual.y) < epsilon);
    assert(std::fabs(expected.z - actual.z) < epsilon);
    assert(std::fabs(expected.w - actual.w) < epsilon);
  }
}

int main() { assert_runtime(); }
