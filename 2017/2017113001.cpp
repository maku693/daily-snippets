...
template <class T>
constexpr std::enable_if_t<is_vec<T>::value, T> operator*(float lhs, const typename T& rhs) noexcept
{
    return rhs * lhs;
}

template <class T>
float distance(const typename T& lhs, const typename T& rhs)
{
    return (lhs - rhs).length();
}

template <class T>
constexpr typename T lerp(const T& a, const T& b, float t)
{
    return a + t * (b - a);
}

struct vec2 : public vec_operators<vec2> {
    float x = 0.0f;
    float y = 0.0f;
...
struct vec3 : public vec_operators<vec3> {
    float x = 0.0f;
    float y = 0.0f;
    float z = 0.0f;
...
struct vec4 : public vec_operators<vec4> {
    float x = 0.0f;
    float y = 0.0f;
    float z = 0.0f;
    float w = 0.0f;
