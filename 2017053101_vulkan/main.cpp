...
{
    const auto sampleCount = vk::SampleCountFlagBits::e1;
    ...
    const auto separatePresentQueue =
        graphicsDevice->getSurfaceSupportKHR(
    ...
    if (separatePresentQueue) {
        ...
        const auto propsList =
            graphicsDevice->getQueueFamilyProperties();
    ...
    const auto device =
        graphicsDevice->createDeviceUnique(deviceCreateInfo);
    ...
    const auto surfaceFormats =
        graphicsDevice->getSurfaceFormatsKHR(*surface);
    ...
    const auto depthTiling = ([&depthFormatProps]() {
        ...
        if (
            depthFormatProps.optimalTilingFeatures & desiredTilingFeature
        ) {
