...
std::vector<uint32_t> findQueueFamilyIndices(const vk::PhysicalDevice& device, const vk::UniqueSurfaceKHR& surface)
{
    ...
    for (int i = 0; i < static_cast<int>(propsList.size()); i++) {
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, PWSTR, int)
{
    ...
    const auto surface = ([hInstance, hWnd, &instance] {
        const auto surfaceCreateInfo = vk::Win32SurfaceCreateInfoKHR().setHinstance(hInstance).setHwnd(hWnd);
        return instance->createWin32SurfaceKHRUnique(surfaceCreateInfo);
    })();

    const auto physicalDeviceAndQueueFamilyIndices = ([&instance, &surface]{
    ...
    const auto device = ([&physicalDevice, &queueFamilyIndices] {
        const auto queueCreateInfo
            ...
            .setQueueFamilyIndex(queueFamilyIndices[0])
            .setQueueCount(1);
    ...
    const auto commandBuffers = ([&device, &queueFamilyIndices] {
        const auto commandPoolCreateInfo
            = vk::CommandPoolCreateInfo().setQueueFamilyIndex(queueFamilyIndices[0]);
   ...
   const auto swapchain = ([&physicalDevice, &device, hWnd, &surface, &queueFamilyIndices] {
       ...
       const auto compositeAlpha = ([&surfaceCapabilities] {
           const std::array<vk::CompositeAlphaFlagBitsKHR, 4> flags
    ...
    const auto buffers = ([&device, &surfaceFormat] {
        ...
        for (const auto& image : images) {
            ...
            buffers.push_back(device->createImageViewUnique(createInfo));
        }
...
std::vector<vk::UniqueCommandBuffer> createCommandBuffervk::UniqueSwapchainKHR createSwapChain
