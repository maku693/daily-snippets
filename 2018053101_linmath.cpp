...
constexpr vec4 operator*(const mat4 &lhs, const vec4 &rhs) noexcept {
  return {lhs[0][0] * rhs.x + lhs[1][0] * rhs.y + lhs[2][0] * rhs.z +
              lhs[3][0] * rhs.w,
          lhs[0][1] * rhs.x + lhs[1][1] * rhs.y + lhs[2][1] * rhs.z +
              lhs[3][1] * rhs.w,
          lhs[0][2] * rhs.x + lhs[1][2] * rhs.y + lhs[2][2] * rhs.z +
              lhs[3][2] * rhs.w,
          lhs[0][3] * rhs.x + lhs[1][3] * rhs.y + lhs[2][3] * rhs.z +
              lhs[3][3] * rhs.w};
}
...
int main() {
  ...
  {
    const auto expected = vec4{2, 4, 6, 1};
    const auto actual = make_translate({1, 2, 3}) * vec4{1, 2, 3, 1};
    assert(actual == expected);
  }

  const auto v2 = make_rotation({0, pi, 0}) * vec4{1, 2, 3, 1};
